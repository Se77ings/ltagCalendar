...
export default function NovoAgendamento({ fecharModal, EditAgendamento, setUpdate }) {
	...
	useEffect(() => {
		if (setUpdate) {
      console.log("Obviamente nao entra aqui ne")
			setUpdate(true);
		}
	}, []);
								...
								},
								{
									...
									onPress: async () => {
										...
										if (EditAgendamento) {
											...
											await criarAgendamento(navigation, values.Nome, values.Telefone, DateString, timeString, data.selectedPrestador, data.selectedServico, setUpdate);
										}
									},
								},
							],
                ...
                {data.prestadores.map((prestador) => (
                  <Picker.Item key={prestador.id} label={prestador.Nome} value={prestador.id} />
							...
							<DropdownSelector
								...
								callbackSelecionados={handleServicoChange}
								selectedItems={data.selectedServico}
								opt={"servico"}
							/>
							...
							<DropdownSelector
								...
								callbackSelecionados={handlePrestadorChange}
								selectedItems={data.selectedPrestador}
								opt={"colaborador"}
							/>
