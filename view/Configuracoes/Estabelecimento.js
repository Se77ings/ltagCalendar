...
// o que falta:
// quando for testar:
// lembrar de inserir uma imagem, remover, ver se atualiza no menu inicial, e inserir outra.

// imagem OK
 //FALTA :
// terminar a edição do estabelecimento
// LEMBRAR DE USAR O TOAST NO FINAL, CASO SUCESSO NA ALTERAÇÃO
...
const Estabelecimento = () => {
	...
	const pickImage = async () => {
		...
		if (!result.canceled) {
			...
			handleSalvar(JSON.stringify(result.assets[0]));
		}
	};

	const handleSalvar = async (image = null) => {
		if (image != null) {
		...
		console.log("imagem é diferente de nulo???? -> ", image != null);
		let formSend = {
			id: estabelecimento.id,
			nome: estabelecimento.Nome,
			telefone: estabelecimento.Telefone.replace(/\D/g, ""),
			endereco: estabelecimento.Endereco,
			logo: image != null ? image : estabelecimento.Logo,
		};
		...
		if (result.success) {
			if (image != null) {
				Toast.show("Imagem alterada com sucesso!", Toast.LONG, { backgroundColor: "#39bf2f", color: "white" });
			} else {
				Toast.show("Dados cadastrados com sucesso!", Toast.LONG, { backgroundColor: "#39bf2f", color: "white" });
			}
			DeviceEventEmitter.emit("atualizarEstabelecimento", { Logo: JSON.stringify(result.assets[0]) });
			setEditar(false);
			return;
		} else {
	...
	return (
		<View contentContainerStyle={styles.container}>
			...
			<View style={styles.scrollContainer}>
				<Text style={{ textAlign: "center" }}>Seu Estabelecimento</Text>
				<Image
