...
const AppointmentSlider = () => {
  ...
  const renderDay = ({ item }) => (
    <TouchableOpacity
      style={[
        styles.dayContainer,
        item.fullDate === selectedDate && styles.selectedDayContainer,
      ]}
...
const agendamentosFiltrados = agendamentos.filter(agendamento => agendamento.data === selectedDate);
  ...
  return (
    <View style={styles.container}>
      {/* Header com logo, nome da loja e botão "Novo Agendamento" */}
      <View style={styles.headerContainer}>
        ...
        onPress={() => navigation.navigate('NovoAgendamento')} // Navega para a página NovoAgendamento
      >
...
<FlatList
  ...
  agendamentosFiltrados.length > 0 ? (
    <FlatList
                    data={agendamentosFiltrados.length > 0 ? agendamentosFiltrados : agendamentos}
                    renderItem={renderAgendamento}
                    keyExtractor={(item) => item.id}
                    contentContainerStyle={styles.lista}
                />
